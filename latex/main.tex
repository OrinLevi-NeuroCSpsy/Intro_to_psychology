...
\begin{enumerate}
    ...
    \item \textbf{הכללה (Generalization)} - תגובה גם לגירויים דומים לגירוי המקורי
    \item \textbf{הבחנה (Discrimination)} - תגובה רק לגירוי הספציפי
\end{enumerate}

\begin{exbox}
\textbf{אלברט הקטן - הכללה:}
\begin{itemize}
    \item ווטסון התנה פחד מעכבר לבן
    \item אלברט פחד גם מ: ארנב לבן, כלב, מסכה עם שיער לבן
    \item זו \textbf{הכללה} - הפחד "התפשט" לגירויים דומים
\end{itemize}
\end{exbox}
...
\subsection{למידת הימנעות (Avoidance Learning)}

\begin{defbox}
\textbf{מה זה?} למידה להימנע מגירוי שמנבא תוצאה שלילית.

\textbf{למה זה בעייתי?} ההימנעות מונעת הכחדה - האדם לא לומד שהגירוי כבר לא מסוכן!
\end{defbox}

\begin{exbox}
\textbf{דוגמה - דינה וחדר האשפה:}
\begin{itemize}
    \item דינה הדליקה אור $\rightarrow$ מנורה התפוצצה (הבהילה אותה)
    \item למדה להימנע מלהדליק אור
    \item גם אחרי שתוקן - ממשיכה להימנע!
    \item \textbf{למה?} לא מנסה להדליק = לא לומדת שזה בטוח עכשיו
\end{itemize}

\textbf{קשר לטיפול:} לכן טיפול בחשיפה (PE) חשוב - שובר את מעגל ההימנעות!
\end{exbox}

\subsection{למידה סמויה (Latent Learning)}

\begin{defbox}
\textbf{אדוארד טולמן} הראה שלמידה יכולה להתרחש \textbf{גם בלי חיזוק}!
\end{defbox}

\begin{exbox}
\textbf{ניסוי המבוך של טולמן:}
\begin{itemize}
    \item חולדות שוטטו במבוך בלי אוכל (לא קיבלו חיזוק)
    \item כשאחר כך הוסיפו אוכל - מצאו אותו \textbf{מיד}!
    \item \textbf{מסקנה:} למדו "מפה קוגניטיבית" של המבוך גם בלי חיזוק
\end{itemize}

\textbf{ערעור על סקינר:} לא כל למידה דורשת חיזוק מיידי!
\end{exbox}
...
\subsection{כלל שיא-סוף (Peak-End Rule)}

\begin{exbox}
\textbf{הניסוי של דניאל כהנמן:}

מטופלים דיווחו על כאב במהלך הליך רפואי (1-10) בכל דקה.

\textbf{מה נמצא?}
\begin{itemize}
    \item הזיכרון מהחוויה נקבע על ידי: \textbf{שיא הכאב} + \textbf{הכאב בסוף}
    \item אורך הפרוצדורה, ממוצע הכאב או הסכום - \textbf{לא משפיעים!}
\end{itemize}

\textbf{השלכות מעשיות:} עדיף שרופא שיניים יסיים כמה דקות אחרי השיא, כשכבר לא כואב - כך הזיכרון יהיה פחות נורא.
\end{exbox}
...
\subsection{Split Brain (מוח מפוצל)}

\begin{defbox}
\textbf{מה זה?} ניתוח חיתוך ה-\textbf{Corpus Callosum} (קורפוס קלוסום) - צרור סיבי העצב שמחבר בין שתי ההמיספרות.

\textbf{למה עושים?} טיפול באפילפסיה קשה.
\end{defbox}

\begin{exbox}
\textbf{ניסוי קלאסי:}
\begin{itemize}
    \item מציגים "מזלג" בשדה ראייה ימני, "כדור" בשדה ראייה שמאלי
    \item שדה ימני $\rightarrow$ המיספרה שמאלית (שפה!)
    \item שדה שמאלי $\rightarrow$ המיספרה ימנית (ללא שפה)
\end{itemize}

\textbf{תוצאה:}
\begin{itemize}
    \item אם שואלים "מה ראית?" $\rightarrow$ יגיד \textbf{"מזלג"} (כי שפה בשמאל)
    \item אם מבקשים להרים ביד שמאל $\rightarrow$ ירים \textbf{כדור} (כי יד שמאל = המיספרה ימנית)
\end{itemize}
\end{exbox}
...
\subsection{חוקרים ישראליים בחקר המוח}

\begin{tabular}{|r|r|r|}
\hline
\rowcolor{tableheader} \textcolor{white}{\textbf{חוקר}} & \textcolor{white}{\textbf{שיטה}} & \textcolor{white}{\textbf{תרומה}} \\
\hline
\rowcolor{tablerow1} ננסי קנוישר & fMRI & אזורי פנים במוח (FFA) \\
\hline
\rowcolor{tablerow2} שלמה בנטין & EEG & גל N170 - תגובה לפנים \\
\hline
\rowcolor{tablerow1} ניצן צנזור & TMS & גרייה מגנטית לחקר קוגניציה \\
\hline
\end{tabular}
...
\subsection{שלבי התפתחות - פרויד ואריקסון}

\begin{tabular}{|r|r|r|}
\hline
\rowcolor{tableheader} \textcolor{white}{\textbf{גיל}} & \textcolor{white}{\textbf{פרויד (פסיכוסקסואלי)}} & \textcolor{white}{\textbf{אריקסון (פסיכוסוציאלי)}} \\
\hline
\rowcolor{tablerow1} 0-1 & אוראלי & אמון vs. חוסר אמון \\
\hline
\rowcolor{tablerow2} 1-3 & אנאלי & אוטונומיה vs. בושה \\
\hline
\rowcolor{tablerow1} 3-6 & פאלי & יוזמה vs. אשמה \\
\hline
\rowcolor{tablerow2} 6-12 & לטנטי & חריצות vs. נחיתות \\
\hline
\rowcolor{tablerow1} 12-18 & גניטלי & זהות vs. בלבול \\
\hline
\end{tabular}
...
\begin{exbox}
\textbf{Home Sign:} ילדים חירשים שלא נחשפו לשפת סימנים מפתחים מערכת תקשורת פרימיטיבית משלהם - "Home Sign". זה מוכיח שיש נטייה מולדת לשפה!
\end{exbox}

\subsubsection{פרויקט פראקאש (Prakash) - פאוואן סינהא}

\begin{exbox}
\textbf{מה זה?} פרויקט בהודו שמנתח ילדים עיוורים מלידה ומחזיר להם ראייה.

\textbf{ממצא:} גם אחרי שנים של עיוורון - יכולת הראייה משתפרת! זה מראה שהמוח \textbf{פלסטי} יותר ממה שחשבו.

\textbf{אבל:} יש דברים שקשה יותר ללמוד - כמו תפיסת עומק ותנועה.
\end{exbox}
...
\subsection{שיטת ההביטואציה (Habituation)}

\begin{defbox}
\textbf{הגדרה:} שיטה לבדיקת הבחנה בין גירויים אצל תינוקות.

\textbf{הפרוצדורה:}
\begin{enumerate}
    \item מציגים גירוי מספר פעמים
    \item התינוק מסתכל פחות ופחות (התרגלות)
    \item מחליפים לגירוי חדש
    \item אם מסתכל יותר זמן = מבחין בין הגירויים!
\end{enumerate}
\end{defbox}

\begin{exbox}
\textbf{דוגמה - האם תכלת נתפס ככחול או ירוק?}
\begin{itemize}
    \item הביטואציה לצבע תכלת
    \item החלפה לכחול או לירוק
    \item תוצאה: תינוקות הסתכלו יותר על ירוק
    \item מסקנה: תכלת נתפס יותר ככחול!
\end{itemize}
\end{exbox}
...
\subsubsection{Other Race Effect (אפקט הגזע האחר)}

\begin{defbox}
\textbf{מה זה?} קושי לזהות ולהבחין בין פנים של אנשים מגזע אחר.

\textbf{למה?} קשור ל-Perceptual Narrowing - נחשפים בעיקר לפנים מהגזע שלנו.
\end{defbox}

\begin{exbox}
\textbf{דוגמה - מסדר זיהוי במשטרה:}
\begin{itemize}
    \item עד קוריאני שגדל בקוריאה $\rightarrow$ יתקשה לזהות חשוד צרפתי
    \item עד קוריאני שגדל בצרפת $\rightarrow$ יזהה טוב יותר (נחשף לפנים מערביים!)
\end{itemize}
\end{exbox}
...
\subsubsection{משימת סאלי ואן (Sally-Anne Task)}

\begin{exbox}
\textbf{סיימון ברון-כהן} - מחקר על אוטיזם:

\textbf{הסיפור:}
\begin{enumerate}
    \item סאלי שמה גולה בסל ויוצאת מהחדר
    \item אן מעבירה את הגולה לקופסה
    \item סאלי חוזרת - איפה תחפש את הגולה?
\end{enumerate}

\textbf{תשובה נכונה:} בסל (שם היא חושבת שהגולה)

\textbf{ממצא:} ילדים עם \textbf{אוטיזם} עונים "בקופסה" - הם לא מבינים שלסאלי יש אמונה שונה משלהם (False Belief).

\textbf{מסקנה:} אוטיזם קשור לפגיעה ב-Theory of Mind.
\end{exbox}
...
\textbf{גורמים שמפחיתים ציות:}
\begin{itemize}
    \item הניסוי מחוץ לאוניברסיטה (פחות סמכותיות)
    \item הנבדק באותו חדר עם הלומד
    \item הוראות בטלפון (לא פנים אל פנים)
    \item הוראות מאדם רגיל (לא מדען)
    \item נוכחות מורד - משתתף שמסרב להמשיך
\end{itemize}

\subsubsection{שחזור ניסוי מילגרם - ג'רי בורגר (2009)}

\begin{exbox}
\textbf{שינויים אתיים:}
\begin{itemize}
    \item הניסוי הסתיים ב-150 וולט
    \item הזכירו לנבדק שהוא יכול לפרוש בכל רגע
    \item הנסיין היה פסיכולוג קליני
\end{itemize}

\textbf{תוצאות:} כ-70\% הסכימו לתת שוק מעל 150 וולט - דומה לניסוי המקורי!

\textbf{שאלוני אישיות:} נבדקו Empathic Concern ו-Need for Control - \textbf{לא נמצא הבדל} בין מי שציית למי שלא.

\textbf{מסקנה:} הסיטואציה משפיעה יותר מתכונות אישיות!
\end{exbox}

\begin{defbox}
\textbf{טעות הייחוס בסיסית (Fundamental Attribution Error):} נטייה לייחס התנהגות של אחרים לאישיותם, ולא למצב שבו הם נמצאים.

\textbf{דוגמה:} אנשים שצופים בניסוי מילגרם חושבים "אני לא הייתי מציית" - מייחסים למאפייני האישיות ולא לכוח הסיטואציה.
\end{defbox}

\subsection{סטריאוטיפים וקוגניציה חברתית}

\begin{defbox}
\textbf{סטריאוטיפ:} אמונות וציפיות לגבי התנהגות של אנשים השייכים לקבוצה מסוימת.
\end{defbox}

\begin{exbox}
\textbf{ניסוי כלי הנשק:}
\begin{itemize}
    \item נבדקים נתבקשו לקבוע אם אדם בתמונה מחזיק נשק או לא
    \item גברים שחורים - יותר טעויות (חפץ ניטרלי נתפס כנשק)
    \item גברים לבנים - פחות טעויות
    \item \textbf{חשוב:} גם נבדקים שחורים וגם לבנים הראו את אותן התוצאות!
\end{itemize}
\end{exbox}

\subsubsection{מבחן IAT (Implicit Association Test)}

\begin{conceptbox}
מבחן שבודק אסוציאציות \textbf{מרומזות} (לא מפורשות) בין קטגוריות.

\textbf{איך עובד?} זמן תגובה מהיר יותר כשהקישור תואם את הסטריאוטיפ הקיים.

\textbf{דוגמה:} גברים + מדעים מדויקים = תגובה מהירה יותר מאשר נשים + מדעים מדויקים.
\end{conceptbox}

\subsection{רושם ראשוני (First Impression)}

\begin{exbox}
\textbf{ניסוי Ambady \& Rosenthal:}
\begin{itemize}
    \item נבדקים צפו ב-30 שניות וידאו של מרצה (ללא קול!)
    \item דירגו: אמפתיה, חום, ביטחון עצמי, מקצועיות
    \item \textbf{תוצאה:} הדירוגים היו דומים לדירוגי הסטודנטים בסוף הסמסטר!
    \item גם 2-5 שניות הספיקו לשיפוט תואם
\end{itemize}

\textbf{מאפיינים שמשפיעים:} הבעות פנים ותנועות גוף (לא אטרקטיביות!)
\end{exbox}

\subsection{Stereotype Threat (קלוד סטיל)}

\begin{defbox}
\textbf{הגדרה:} חשש שביצועים יאוששו סטריאוטיפ שלילי - מה שבסופו של דבר פוגע בביצועים.

\textbf{מי פיתח?} קלוד סטיל (Claude Steele)

\textbf{דוגמה:} נשים במתמטיקה - כשמזכירים הבדלים מגדריים, הביצוע יורד.
\end{defbox}

\subsection{מודל תוכן הסטריאוטיפ (Stereotype Content Model)}

\begin{conceptbox}
\textbf{פיתחו:} סוזן פיסקה, פיטר גליק ואמי קאדי

\textbf{שני מימדים:}
\begin{enumerate}
    \item \textbf{חום (Warmth)} - האם הם טובים? מתכוונים לטוב?
    \item \textbf{יכולת (Competence)} - האם הם מסוגלים?
\end{enumerate}

\textbf{ארבע קומבינציות:}
\begin{tabular}{|r|r|r|}
\hline
\rowcolor{tableheader} \textcolor{white}{\textbf{חום}} & \textcolor{white}{\textbf{יכולת}} & \textcolor{white}{\textbf{דוגמה}} \\
\hline
\rowcolor{tablerow1} גבוה & גבוה & הקבוצה שלי (in-group) \\
\hline
\rowcolor{tablerow2} גבוה & נמוך & קשישים, נכים (רחמים) \\
\hline
\rowcolor{tablerow1} נמוך & גבוה & יהודים, אסייתים (קנאה) \\
\hline
\rowcolor{tablerow2} נמוך & נמוך & חסרי בית, מכורים (בוז) \\
\hline
\end{tabular}
\end{conceptbox}
...
\subsection{שיטות אבחון פסיכולוגי}

\begin{tabular}{|r|r|r|}
\hline
\rowcolor{tableheader} \textcolor{white}{\textbf{שיטה}} & \textcolor{white}{\textbf{יתרון}} & \textcolor{white}{\textbf{חיסרון}} \\
\hline
\rowcolor{tablerow1} שאלון דיווח עצמי & מהיר, מבוסס מחקר & המטופל יכול להטות תשובות \\
\hline
\rowcolor{tablerow2} מבחני השלכה (רורשאך) & קשה להטות & עקביות נמוכה בין מאבחנים \\
\hline
\end{tabular}

\begin{defbox}
\textbf{מבחני השלכה:} גירויים עמומים (כתמי דיו) - המטופל "משליך" את עולמו הפנימי.

\textbf{דוגמה:} מבחן רורשאך - מה אתה רואה בכתם?

\textbf{בעיה:} מאבחנים שונים מגיעים למסקנות שונות (מהימנות נמוכה).
\end{defbox}
...
\subsection{הגדרת הפרעה נפשית}

\begin{defbox}
\textbf{הגדרת הפרעה מנטלית לפי ה-DSM:}

סינדרום (אוסף סימפטומים) המאופיין בהפרעה משמעותית בקוגניציה, ויסות רגשי או התנהגות, הבאה לידי ביטוי בחוסר תפקוד של תהליכים פסיכולוגיים, ביולוגיים או התפתחותיים.

\textbf{מאפיינים:}
\begin{itemize}
    \item סבל משמעותי
    \item חוסר תפקוד משמעותי בפעילות חברתית/תעסוקתית/יומיומית
\end{itemize}
\end{defbox}
...
ספר שמוציא ארגון הפסיכיאטרים האמריקאי. כולל כ-300 הפרעות נפשיות.

\textbf{למה צריך DSM?}
\begin{itemize}
    \item להציע טיפול מתאים
    \item להבין את מצוקת המטופל
    \item תקשורת בין אנשי מקצוע
    \item מחקר - הגדרת אוכלוסייה
    \item קבלת שירותי רווחה וביטוח
\end{itemize}
\end{defbox}
...
\subsubsection{הפרעת פאניקה - דוגמה מה-DSM}

\begin{exbox}
\textbf{הגדרה:} התקפות פאניקה בלתי צפויות וחוזרות.

\textbf{התקפת פאניקה:} עלייה חדה בתחושת פחד שמגיעה לשיא תוך דקות, עם לפחות 4 מהסימפטומים:
\begin{itemize}
    \item עלייה בדופק, הזעה, רעד
    \item קוצר נשימה, תחושת חנק
    \item כאב בחזה, בחילה, סחרחורת
    \item תחושת "משתגע" או חשש ממוות
\end{itemize}

\textbf{חשוב:} אנשים רבים מגיעים לחדר מיון כי חושבים שזה התקף לב!

\textbf{בעקבות ההתקף:} חשש מהתקף נוסף, הימנעות ממקומות/מצבים שעלולים לגרום להתקף.
\end{exbox}

\subsection{אבחון הפרעות נפשיות}

\begin{conceptbox}
\textbf{כלי מדידה חייבים להיות:}
\begin{itemize}
    \item \textbf{מהימנים} - תוצאה דומה בכל פעם שמודדים
    \item \textbf{תקפים} - מודדים את מה שאמורים למדוד
\end{itemize}

\textbf{בעיה:} אין עדיין בדיקות פיזיולוגיות (MRI, EEG) שמספיק רגישות לאבחון!
\end{conceptbox}

\subsubsection{ראיון מובנה - MINI}

\begin{defbox}
\textbf{MINI} = Mini International Neuropsychiatric Interview

ראיון מובנה עם שאלות קבועות והוראות מדויקות - מגביר מהימנות בין מאבחנים.

\textbf{למה עדיף מראיון חופשי?} בראיון חופשי - מטפלים שונים שואלים שאלות שונות ומגיעים לאבחנות שונות.
\end{defbox}

\subsubsection{מבחן רורשאך - הויכוח}

\begin{warnbox}
\textbf{הבעיה:} ככל שהפסיכולוג מוסיף \textbf{פרשנות} - התוקף \textbf{יורד}!

\textbf{Exner's Comprehensive System:} שיטת ציינון מדוקדקת שמגבירה מהימנות.

\textbf{מה תקף?}
\begin{itemize}
    \item הבחנה בין פסיכוטי ללא-פסיכוטי - \textbf{תקף}
    \item מורכבות אינטלקטואלית - \textbf{תקף}
    \item פרשנות חופשית של הפסיכולוג - \textbf{לא תקף!}
\end{itemize}
\end{warnbox}
...
\subsubsection{הגל השלישי: ACT ומיינדפולנס}

\begin{conceptbox}
\textbf{ACT} = Acceptance and Commitment Therapy

\textbf{הרעיון:} לא ניתן לשלוט במחשבות! במקום לנסות לשנות אותן - לקבל אותן.

\textbf{דוגמה:} "אל תחשוב על דוב לבן" - ככל שמנסים יותר, חושבים יותר!
\end{conceptbox}

\begin{tabular}{|r|r|r|}
\hline
\rowcolor{tableheader} \textcolor{white}{\textbf{היבט}} & \textcolor{white}{\textbf{CBT קלאסי}} & \textcolor{white}{\textbf{ACT (גל שלישי)}} \\
\hline
...
\subsubsection{מיינדפולנס (Mindfulness) - קשיבות}

\begin{defbox}
\textbf{מה זה?} מודעות בהירה ולא שיפוטית למתרחש ברגע הנוכחי.

\textbf{מי פיתח?} ג'ון קבט-זין (Jon Kabat-Zinn) מאוניברסיטת מסצ'וסטס.

\textbf{MBSR} = Mindfulness Based Stress Reduction (8 מפגשים, חודשיים)
\end{defbox}

\begin{exbox}
\textbf{ממצאי מחקר (ד"ר ריקרדו טרש):}
\begin{itemize}
    \item ירידה ברמות לחץ וחרדה
    \item שיפור בתפקוד המערכת החיסונית
    \item שיפור ביכולות קשב
    \item שינויים מוחיים: פחות פעילות ב-Default Mode Network
\end{itemize}

\textbf{מחקר עם סטודנטים עם ADHD:} שיפור בקשב מתמשך, בקרת קשב וקשב סלקטיבי.
\end{exbox}

\subsubsection{כלי מדידה: BDI}

\begin{defbox}
\textbf{BDI = Beck Depression Inventory}

שאלון דיווח עצמי שפיתח \textbf{אהרון בק} למדידת רמת דיכאון.

21 שאלות, 0-3 נקודות לכל שאלה, טווח ציונים: 0-63.

\textbf{שימוש:} לפני ואחרי טיפול - האם הטיפול עזר?
\end{defbox}

\subsection{בדיקת יעילות טיפול}

\begin{conceptbox}
\textbf{מה נדרש כדי לבדוק יעילות טיפול?}
\begin{enumerate}
    \item \textbf{פרוטוקול אחיד} - טיפול דומה לכל המטופלים
    \item \textbf{חלוקה רנדומלית} - לקבוצת טיפול וביקורת
    \item \textbf{כלי מדידה אחיד} - מדד מספרי לסימפטומים
\end{enumerate}
\end{conceptbox}

\begin{defbox}
\textbf{אפקט הפלסבו:} האמונה שמקבלים טיפול יכולה בפני עצמה להביא לשיפור!

\textbf{שאלה:} האם הטיפול שלנו טוב \textbf{יותר} מפלסבו?
\end{defbox}

\begin{exbox}
\textbf{מטא-אנליזה - טיפול פסיכודינמי קצר לדיכאון:}
\begin{itemize}
    \item 23 מחקרים, מעל 1000 מטופלים
    \item \textbf{תוצאה:} שיפור בסימפטומים שנשמר גם שנה אחרי הטיפול
    \item יעיל באותה מידה כמו CBT
\end{itemize}
\end{exbox}

\subsection{הברית הטיפולית}

\begin{defbox}
\textbf{מה חשוב בבחירת מטפל? (פרופ' חיים עומר)}
\begin{itemize}
    \item \textbf{התאמה אישית} - סימפטיה הדדית, התאמה במזג ובשפה
    \item \textbf{הסכמה על יעדים} - מטפל שמקשיב ליעדים שלך
    \item \textbf{ברית טיפולית} - הסכמה על מה רוצים להשיג
\end{itemize}

\textbf{חשוב:} רק במקרים ספציפיים (פוביות, OCD, בעיות מיניות) יש עדיפות לסוג טיפול מסוים. ברוב המקרים - הברית הטיפולית חשובה יותר מהגישה!
\end{defbox}
